...
# 라우터 import
from meals.imagetest import router as food_analysis_router
...
# 🍽️ 음식 분석 기능은 라우터로 이동됨 (routers/food_analysis.py)
...
def root():
    return {
        ...
        "features": [
            "운동 & 영양 챗봇 (/ask)",
            "음식 이미지 분석 (/api/food/analyze) - 라우터 기반",
            "음식 텍스트 분석 (/api/food/analyze/text) - 라우터 기반",
            "웹 크롤링 (/crawl)",
            "시스템 상태 (/health)"
        ],
        "routers": [
            "food_analysis - 음식 분석 기능"
        ]
    }
